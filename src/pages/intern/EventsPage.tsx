...
import { 
    ...
    GraduationCap, BookOpen, Pin, ChevronRight, 
...
interface SelectedEvent extends EventItem {
    // Modal-specific merged properties
}
...
const EventsSkeleton: React.FC = () => {
    return (
        <div className="p-4 md:p-6 bg-slate-100 min-h-screen font-sans flex flex-col gap-5">
            {/* Header Skeleton */}
...
const EventsPage: React.FC = () => {
    ...
    const loadData = useCallback(async (): Promise<EventItem[]> => {
        try {
            ...
            const sortedEvents = response.data.sort((a, b) => new Date(b.start).getTime() - new Date(a.start).getTime());
            return sortedEvents;
        } catch (err) {
    ...
    const getTypeStyles = (type?: string): TypeStyles => {
        switch(type?.toLowerCase()) {
            case 'holiday':
                return { border: 'border-blue-500', badge: 'bg-blue-100 text-blue-700', iconBg: 'bg-blue-50 text-blue-500' };
            case 'reminder':
                return { border: 'border-green-500', badge: 'bg-green-100 text-green-700', iconBg: 'bg-green-50 text-green-500' };
            case 'event':
            default:
                return { border: 'border-yellow-500', badge: 'bg-yellow-100 text-yellow-700', iconBg: 'bg-yellow-50 text-yellow-500' };
        }
    };
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="flex-grow">
                                            ...
                                            <h3 className="text-lg font-bold text-slate-800 mb-1 leading-tight group-hover:text-[#0B1EAE] transition-colors">
                                                {ev.title}
                                            </h3>
                            ...
                            {recentEvents.map(ev => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <h3 className="text-base font-bold text-slate-800 mb-2 leading-tight group-hover:text-[#0B1EAE] transition-colors">
                                            {ev.title}
                                        </h3>
                ...
                >
                    ...
                    >
                        ...
                        <div className="p-6 border-t border-slate-100 bg-slate-50 rounded-b-2xl flex justify-end">
                            <button 
                                onClick={() => setIsModalOpen(false)} 
                                className="px-8 py-2.5 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-lg transition-colors cursor-pointer border-none"
                            >
                                Close
                            </button>
                        </div>
